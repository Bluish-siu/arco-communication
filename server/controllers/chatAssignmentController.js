...
// Exportable internal evaluator for direct server-side invocation
export async function evaluateAssignmentInternal({
  contactName,
  contactPhone,
  contactEmail,
  tag,
...
export const chatAssignmentController = {
  ...
  evaluateAssignment: async (req, res, next) => {
    try {
      const userId = req.user?.id || 'usr_1';
      const data = await evaluateAssignmentInternal({ ...req.body, userId });
