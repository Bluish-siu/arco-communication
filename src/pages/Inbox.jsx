...
export default function Inbox() {
  ...
  // Live background polling for inbound WhatsApp messages & delivery statuses
  useEffect(() => {
    const pollInterval = setInterval(() => {
      if (typeof document !== 'undefined' && document.visibilityState === 'visible') {
        loadConversations(appliedFilters);
      }
    }, 3500);
                          ...
                          {selectedChat.channel === 'whatsapp' && (
                            <span
                              className={`px-2 py-0.5 rounded-full text-[10px] font-bold border flex items-center gap-1 ${
                                selectedChat.responseWindow === 'active'
                                  ? 'bg-emerald-50 text-emerald-700 border-emerald-200'
                                  : 'bg-amber-50 text-amber-700 border-amber-200'
                              }`}
                              title={
                                selectedChat.responseWindow === 'active'
                                  ? 'WhatsApp 24-hour service window is active'
                                  : 'WhatsApp 24-hour service window has expired'
                              }
                            >
                        ...
                        >
                          ...
                          <div className="flex items-center gap-1 mt-1 px-1 text-[10px] text-slate-400">
                            ...
                            {isMe && (
                              <>
                                {msg.status === 'read' ? (
                                  <CheckCheck className="w-3.5 h-3.5 text-sky-500" title="Read" />
                                ) : msg.status === 'delivered' ? (
                                  <CheckCheck className="w-3.5 h-3.5 text-slate-400" title="Delivered" />
                                ) : msg.status === 'failed' ? (
                                  <span className="flex items-center gap-0.5 text-rose-500 font-semibold" title={msg.errorMessage || 'Failed to deliver'}>
                          ...
                          {isMe && msg.status === 'failed' && msg.errorMessage && (
                            <p className="text-[10px] text-rose-500 mt-0.5 max-w-[85%] text-right font-medium">
                              {msg.errorMessage}
                            </p>
                          )}
                        </div>
