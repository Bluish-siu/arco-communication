...
export default function CustomReplySetupModal({
  isOpen,
  onClose,
  templateButtons = [],
  currentConfig = {},
  ...
  const [messageText, setMessageText] = useState(
    'Sure, someone from our team will get in touch shortly.'
  );
    ...
    if (isOpen) {
      ...
      setTriggerButton(
        currentConfig.triggerButton ||
          templateButtons.find((b) => /help|support|contact|talk/i.test(b.text || b))?.text ||
          (templateButtons[0]?.text || templateButtons[0] || 'Help')
      );
      setMessageText(
        currentConfig.messageText ||
          'Sure, someone from our team will get in touch shortly.'
      );
    }
  ...
  const handleSave = () => {
    onSave({
      enabled: true,
      triggerType,
      triggerButton: triggerButton || 'Help',
  ...
  if (!availableButtons.includes('Speak to Agent')) availableButtons.push('Speak to Agent');
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl overflow-hidden border border-gray-100 animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="bg-[#0d3b30] px-5 py-3.5 flex items-center justify-between text-white">
          <div className="flex items-center gap-2.5">
            ...
            <h3 className="font-bold text-sm">Setup Custom Message flow</h3>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="p-6 space-y-5">
          <p className="text-xs text-gray-600 leading-relaxed">
            If the customer's reply matches the below trigger, a <strong className="text-gray-900 font-semibold">custom reply</strong> will be sent automatically.
          </p>
              ...
              <div className="space-y-2 pt-1">
                ...
                <div className="border border-gray-200 rounded-lg p-3 bg-gray-50/50 focus-within:border-[#0d3b30] focus-within:bg-white transition-colors">
                  ...
                  <div className="flex justify-between items-center pt-2 border-t border-gray-100 text-[11px] text-gray-400">
                    <span className="italic">Plain text response</span>
                    <span>{messageText.length}/999</span>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="bg-[#efeae2] rounded-lg p-3 space-y-2.5 shadow-inner min-h-[220px] flex flex-col justify-end text-xs">
                ...
                <div className="bg-white p-2.5 rounded-lg rounded-tl-none shadow-xs text-gray-800 self-start max-w-[85%] border border-black/5">
                  <p className="text-[11px] leading-relaxed">
                    Here is a 20% discount code for your next order! Need help? Tap below:
                  </p>
                ...
                {/* Auto Custom Reply Bubble */}
                {messageText && (
                  <div className="bg-white p-2.5 rounded-lg rounded-tl-none shadow-xs text-gray-800 self-start max-w-[85%] border border-black/5">
                    <p className="text-[11px] leading-relaxed">{messageText}</p>
